...
window.addEventListener('DOMContentLoaded',(event) => {
  fetch(typeof jsonURL!=='undefined'? jsonURL:"./assets/data/adventure.json").then(response => response.json()).then(json => loadAdventure(json));
});
...
const loadPage=() => {
  ...
  const timedate=new Date(pageData.d).toLocaleTimeString(locale,options)
  ...
  loadIntoElement("timestamp",MSPFA.parseBBCode(timedate.replace(/,/,'')))
  loadIntoElement("button",MSPFA.parseBBCode(pageData.btn))
  loadIntoElement("author",MSPFA.parseBBCode(pageData.comm))
  loadIntoElement("user",MSPFA.parseBBCode(pageData.user))
...
const loadLog=() => {
  ...
  adventureData.p.filter(p => p.d<new Date().getTime()).forEach((pageData,index) => {
    ...
    const date=new Date(pageData.d).toLocaleDateString(datelocale,dateoptions)
    ...
    li.append(document.createTextNode(date.replace(/,/,'')+" - "))
    li.append(a)
  ...
  loadIntoElement("command",MSPFA.parseBBCode(adventureData.n))
